...
// 拷贝 newer 文件
let sourcePath_old = 'H:/next911fresh/next';
let targetPath_old = 'H:/next911fresh/my-app/node_modules/next';

// __dirname 是 H:/next911fresh/next （build.js 所在目录）
///=== path.resolve(__dirname)：解析为 H:/next911fresh/next
let sourcePath = path.resolve(__dirname);  ///===__dirname：表示当前执行的 JS 文件所在的目录（即 next/build.js）。
let targetPath = path.resolve(__dirname, '../my-app/node_modules/next'); ///=== 解析为 H:/next911fresh/my-app/node_modules/next
...
async function babelCompileDir(srcDir, outDir, { isClient = false, stripExtension = false } = {}) {
  ...
  await Promise.all(files.map(async (file) => {
    ...
    const options = {
      ...
      compact: false, ///===!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ,  set it to true for production 
      babelrc: false,
      configFile: false,
      filename: file
    };
