...
async function _default(dir, options, configuration) {
  ...
  const pagesManifest = !options.pages && require((0, _path.join)(distDir, _constants2.SERVER_DIRECTORY, _constants2.PAGES_MANIFEST));
  let prerenderManifest;
  try {
    prerenderManifest = require((0, _path.join)(distDir, _constants2.PRERENDER_MANIFEST));
  } catch (_) {}
  const distPagesDir = (0, _path.join)(distDir, isLikeServerless ? _constants2.SERVERLESS_DIRECTORY : (0, _path.join)(_constants2.SERVER_DIRECTORY, 'static', buildId), 'pages');
  ...
  for (const page of pages) {
    ...
    if (page === '/_document' || page === '/_app' || page === '/_error' || page.match(_constants.API_ROUTE)) {
      continue;
    }
    defaultPathMap[page] = {
      page
  ...
  if (outDir === (0, _path.join)(dir, 'public')) {
    throw new Error(`The 'public' directory is reserved in Next.js and can not be used as the export out directory. https://err.sh/zeit/next.js/can-not-output-to-public`);
  }
  ...
  if ((0, _fs.existsSync)((0, _path.join)(distDir, _constants2.CLIENT_STATIC_FILES_PATH))) {
    ...
    await (0, _recursiveCopy.recursiveCopy)((0, _path.join)(distDir, _constants2.CLIENT_STATIC_FILES_PATH), (0, _path.join)(outDir, '_next', _constants2.CLIENT_STATIC_FILES_PATH));
  }

  // Get the exportPathMap from the config file
  if (typeof nextConfig.exportPathMap !== 'function') {
    console.log(`> No "exportPathMap" found in "${_constants2.CONFIG_FILE}". Generating map from "./pages"`);
  ...
  if (hasApiRoutes) {
    log(_chalk.default.yellow('  API pages are not supported by next export. https://err.sh/zeit/next.js/api-routes-static-export'));
  }
  const progress = !options.silent && createProgress(filteredPaths.length);
  const sprDataDir = options.buildExport ? outDir : (0, _path.join)(outDir, '_next/data');
  ...
  const worker = new _jestWorker.default(require.resolve('./worker'), {
    maxRetries: 0,
    numWorkers: threads,
  ...
  await Promise.all(filteredPaths.map(async path => {
    ...
    for (const validation of result.ampValidations || []) {
      ...
      hadValidationError = hadValidationError || Array.isArray(result && result.errors) && result.errors.length > 0;
    }
  ...
  if (!options.buildExport && prerenderManifest) {
    await Promise.all(Object.keys(prerenderManifest.routes).map(async route => {
      ...
      const htmlDest = (0, _path.join)(outDir, `${route}.html`);
      const jsonDest = (0, _path.join)(sprDataDir, `${route}.json`);
