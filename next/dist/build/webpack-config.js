...
async function getBaseWebpackConfig(dir, {
    ...
    prodGranular: {
      ...
      cacheGroups: {
        ...
        lib: {
          ...
          name(module) {
            return _crypto.default.createHash('sha1').update(module.libIdent({
              context: dir
  ...
  let customAppFile = config.experimental.css ? await (0, _findPageFile.findPageFile)(_path.default.join(dir, 'pages'), '/_app', config.pageExtensions) : null;
  if (customAppFile) {
    customAppFile = _path.default.resolve(_path.default.join(dir, 'pages', customAppFile));
  }
  let webpackConfig = {
    devtool,
    mode: webpackMode,
    ...
    externals: !isServer ? undefined : !isServerless ? [(context, request, callback) => {
      ...
      try {
        ...
        if (request === 'react-ssr-prepass' && !config.experimental.ampBindInitData) {
      ...
      if (!res.match(/next[/\\]dist[/\\]next-server[/\\]/) && (res.match(/next[/\\]dist[/\\]/) || res.match(/node_modules[/\\]@babel[/\\]runtime[/\\]/) || res.match(/node_modules[/\\]@babel[/\\]runtime-corejs2[/\\]/))) {
        return callback();
      }
    ...
    output: {
      ...
      filename: ({
        ...
        if (!dev && (chunk.name === _constants2.CLIENT_STATIC_FILES_RUNTIME_MAIN || chunk.name === _constants2.CLIENT_STATIC_FILES_RUNTIME_WEBPACK)) {
    ...
    resolveLoader: {
      ...
      alias: ['emit-file-loader', 'error-loader', 'next-babel-loader', 'next-client-pages-loader', 'next-data-loader', 'next-serverless-loader', 'noop-loader'].reduce((alias, loader) => {
    ...
    module: {
      ...
      rules: [config.experimental.ampBindInitData && !isServer && {
        test: /\.(tsx|ts|js|mjs|jsx)$/,
        include: [_path.default.join(dir, 'data')],
        use: 'next-data-loader'
      }, {
        test: /\.(tsx|ts|js|mjs|jsx)$/,
        include: [dir, /next[\\/]dist[\\/]next-server[\\/]lib/, /next[\\/]dist[\\/]client/, /next[\\/]dist[\\/]pages/, /[\\/](strip-ansi|ansi-regex)[\\/]/],
        exclude: path => {
          if (/next[\\/]dist[\\/]next-server[\\/]lib/.test(path) || /next[\\/]dist[\\/]client/.test(path) || /next[\\/]dist[\\/]pages/.test(path) || /[\\/](strip-ansi|ansi-regex)[\\/]/.test(path)) {
            return false;
          }
      ...
      // Support CSS imports
      {
        oneOf: [{
          test: /\.css$/,
          issuer: {
            include: [customAppFile].filter(Boolean)
          },
          use: isServer ?
          ...
          test: /\.css$/,
          use: isServer ? require.resolve('ignore-loader') : {
            loader: 'error-loader',
            options: {
              reason: `Global CSS ${_chalk.default.bold('cannot')} be imported from files other than your ${_chalk.default.bold('Custom <App>')}. Please move all global CSS imports to ${_chalk.default.cyan(customAppFile ? _path.default.relative(dir, customAppFile) : 'pages/_app.js')}.\n` + `Read more: https://err.sh/next.js/global-css`
            }
          }
        }]
      }, config.experimental.css && {
    ...
    }), !isServer && new _nextDropClientPagePlugin.DropClientPage(), new _chunkGraphPlugin.ChunkGraphPlugin(buildId, {
      dir,
      distDir,
      isServer
    }),
    ...
    config.future.excludeDefaultMomentLocales && new _webpack.default.IgnorePlugin(/^\.\/locale$/, /moment$/), ...(dev ? (() => {
    ...
    config.experimental.css && !isServer && !dev && new _miniCssExtractPlugin.default({
      filename: 'static/css/[contenthash].css',
      chunkFilename: 'static/css/[contenthash].chunk.css'
    }), tracer && new _profilingPlugin.ProfilingPlugin({
      tracer
    }), !isServer && useTypeScript && new _forkTsCheckerWebpackPlugin.default(_pnpWebpackPlugin.default.forkTsCheckerOptions({
  ...
  if (webpackConfig.module && Array.isArray(webpackConfig.module.rules)) {
    ...
    [].forEach.call(webpackConfig.module.rules, function (rule) {
      ...
      [].forEach.call(rule.use, function (use) {
        ...
        use.loader === 'css-loader' || use.loader === 'css-loader/locals') && use.options && typeof use.options === 'object' &&
        ...
        try {
          ...
          // or Less plugin.
          const correctNextCss = (0, _resolveRequest.resolveRequest)('@zeit/next-css', isCss ?
