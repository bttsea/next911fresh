  ...
  apply(compiler) {
    compiler.hooks.emit.tap('NextJsPagesManifest', compilation => {
      ...
      for (const chunk of chunks) {
        const result = (this.serverless ? _constants.SERVERLESS_ROUTE_NAME_REGEX : _constants.ROUTE_NAME_REGEX).exec(chunk.name);
        if (!result) {
          continue;
        }
      ...
      compilation.assets[_constants.PAGES_MANIFEST] = new _webpackSources.RawSource(JSON.stringify(pages));
    });
  }
}
