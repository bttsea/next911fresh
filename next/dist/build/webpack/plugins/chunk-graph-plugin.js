...
exports.ChunkGraphPlugin = void 0;
exports.getPageChunks = getPageChunks;
...
function getPageChunks(page) {
  ...
  [...(manifest.pages[page] || []), ...(pageModules[page] || [])].map(mod => {
    ...
    if (mod.includes('node_modules/')) {
      if (mod.match(/node_modules\/(@babel|core-js|styled-jsx|string-hash|object-assign|process|react|react-dom|scheduler|regenerator-runtime|webpack|node-libs-browser)\//)) {
        return null;
      }
...
function getFiles(dir, modules) {
  ...
  function getFileByIdentifier(id) {
    ...
    if (id && !_path.default.isAbsolute(id)) {
      id = _path.default.resolve(dir, id);
    }
    return id;
  }
  return modules.reduce((acc, val) => val.modules ? acc.concat(getFiles(dir, val.modules)) : (acc.push(getFileByIdentifier(typeof val.identifier === 'function' ? val.identifier() : val.identifier)), acc), []).filter(Boolean);
}
  ...
  apply(compiler) {
    ...
    compiler.hooks.emit.tap('ChunkGraphPlugin', compilation => {
      ...
      compilation.chunks.forEach(chunk => {
        ...
        .filter(val => _path.default.relative(this.distDir, val).startsWith('..'))
        // convert from absolute path to be portable across operating systems
        // and directories
        .map(f => _path.default.relative(dir, f));
        let pageName;
        if (chunk.entryModule && chunk.entryModule.loaders) {
          ...
          if (entryLoader) {
            const {
              page
            } = (0, _querystring.parse)(entryLoader.options);
        ...
        if (pageName) {
          ...
          if (chunk.name === _constants.CLIENT_STATIC_FILES_RUNTIME_MAIN) {
            sharedFiles.push(...files);
            sharedChunks.push(...involvedChunks);
          } else {
            manifest.chunks[chunk.name] = [...new Set([...(manifest.chunks[chunk.name] || []), ...files])].sort();
          }
        }
      });
      const getLambdaChunk = name => name.includes(this.buildId) ? name.replace(new RegExp(`${this.buildId}[\\/\\\\]`), 'client/').replace(/[.]js$/, `.${this.buildId}.js`) : name;
      ...
      for (const page in pages) {
        ...
        if (!this.isServer) {
          manifest.pageChunks[page] = [...new Set([...(manifest.pageChunks[page] || []), ...pageChunks[page], ...pageChunks[page].map(getLambdaChunk), ...sharedChunks, ...sharedChunks.map(getLambdaChunk)])].sort();
        }
      }
    });
  }
