...
exports.UnlinkRemovedPagesPlugin = void 0;
...
class UnlinkRemovedPagesPlugin {
  constructor() {
    this.prevAssets = void 0;
    this.prevAssets = {};
  }
  apply(compiler) {
    compiler.hooks.afterEmit.tapAsync('NextJsUnlinkRemovedPages', (compilation, callback) => {
      const removed = Object.keys(this.prevAssets).filter(a => _constants.IS_BUNDLED_PAGE_REGEX.test(a) && !compilation.assets[a]);
      this.prevAssets = compilation.assets;
      Promise.all(removed.map(async f => {
        const path = (0, _path.join)(compiler.outputPath, f);
        try {
          await unlink(path);
        } catch (err) {
