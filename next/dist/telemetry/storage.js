...
let isDisabled = !!NEXT_TELEMETRY_DISABLED;
function notify() {
...
function setup() {
  ...
  const cwd = (ciEnvironment.isCI || (0, _isDocker.default)()) && distDir ? _path.default.join(distDir, 'cache') : undefined;
  config = new _conf.default({
    projectName: 'nextjs',
    cwd
  });
  let anonymousId = config.get(TELEMETRY_KEY_ID);
  if (!anonymousId) {
    config.set(TELEMETRY_KEY_ID, anonymousId = (0, _crypto.randomBytes)(32).toString('hex'));
  }
  if (!config.get(TELEMETRY_KEY_SALT)) {
    config.set(TELEMETRY_KEY_SALT, (0, _crypto.randomBytes)(16).toString('hex'));
  }
  projectId = (0, _projectId.getProjectId)();
  randomRunId = (0, _crypto.randomBytes)(8).toString('hex');
  if (config.get(TELEMETRY_KEY_ENABLED, true) === false) {
    isDisabled = true;
  }
  notify();
}
function setDistDir(_distDir) {
  distDir = _distDir;
}
