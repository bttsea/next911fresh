...
const matchNextPageBundleRequest = (0, _router.route)('/_next/static/:buildId/pages/:path*.js(.map)?');
...
function erroredPages(compilation, options = {
  ...
  for (const error of compilation.errors) {
    ...
    if (!_constants2.IS_BUNDLED_PAGE_REGEX.test(name)) {
      continue;
    }
...
class HotReloader {
  constructor(dir, {
    ...
    this.webpackDevMiddleware = void 0;
  ...
  async run(req, res, parsedUrl) {
    ...
    const handlePageBundleRequest = async (res, parsedUrl) => {
      ...
      if (page === '/_error' || _constants2.BLOCKED_PAGES.indexOf(page) === -1) {
        ...
        const bundlePath = (0, _path.join)(this.dir, this.config.distDir, 'static/development/pages', page + '.js');
  ...
  async clean() {
    return (0, _recursiveDelete.recursiveDelete)((0, _path.join)(this.dir, this.config.distDir));
  }
  async getWebpackConfig() {
    ...
    const entrypoints = (0, _entries.createEntrypoints)(pages, 'server', this.buildId, this.config);
    let additionalClientEntrypoints = {};
    additionalClientEntrypoints[_constants2.CLIENT_STATIC_FILES_RUNTIME_AMP] = `.${_path.sep}` + (0, _path.relative)(this.dir, (0, _path.join)(_constants.NEXT_PROJECT_ROOT_DIST_CLIENT, 'dev', 'amp-dev'));
  ...
  assignBuildTools({
    ...
    this.middlewares = [webpackDevMiddleware,
    // must come before hotMiddleware
    onDemandEntries.middleware(), webpackHotMiddleware, (0, _errorOverlayMiddleware.default)({
      dir: this.dir
    })];
  }
  async prepareBuildTools(multiCompiler) {
    const tsConfigPath = (0, _path.join)(this.dir, 'tsconfig.json');
    const useTypeScript = await (0, _fileExists.fileExists)(tsConfigPath);
    (0, _output.watchCompilers)(multiCompiler.compilers[0], multiCompiler.compilers[1], useTypeScript, ({
      errors,
      warnings
    }) => this.send('typeChecked', {
      errors,
      warnings
    }));
    ...
    multiCompiler.compilers[1].hooks.done.tap('NextjsHotReloaderForServer', stats => {
      ...
      const documentChunk = compilation.chunks.find(c => c.name === (0, _path.normalize)(`static/${this.buildId}/pages/_document.js`));
    ...
    multiCompiler.compilers[0].hooks.done.tap('NextjsHotReloaderForClient', stats => {
      ...
      const chunkNames = new Set(compilation.chunks.map(c => c.name).filter(name => _constants2.IS_BUNDLED_PAGE_REGEX.test(name)));
      if (this.initialized) {
        ...
        if (addedPages.size > 0) {
          for (const addedPage of addedPages) {
            let page = '/' + _constants2.ROUTE_NAME_REGEX.exec(addedPage)[1].replace(/\\/g, '/');
        ...
        if (removedPages.size > 0) {
          for (const removedPage of removedPages) {
            let page = '/' + _constants2.ROUTE_NAME_REGEX.exec(removedPage)[1].replace(/\\/g, '/');
            page = page === '/index' ? '/' : page;
    ...
    const webpackDevMiddleware = (0, _webpackDevMiddleware.default)(multiCompiler, webpackDevMiddlewareConfig);
    const webpackHotMiddleware = (0, _webpackHotMiddleware.default)(multiCompiler.compilers[0], {
      path: '/_next/webpack-hmr',
      log: false,
      heartbeat: 2500
    });
    const onDemandEntries = (0, _onDemandEntryHandler.default)(webpackDevMiddleware, multiCompiler, {
      dir: this.dir,
      buildId: this.buildId,
      pagesDir: this.pagesDir,
  ...
  async getCompilationErrors(page) {
    const normalizedPage = (0, _onDemandEntryHandler.normalizePage)(page);
    ...
    if (this.stats.hasErrors()) {
      ...
      const failedPages = erroredPages(compilation, {
        enhanceName(name) {
          return '/' + _constants2.ROUTE_NAME_REGEX.exec(name)[1];
        }
      });
