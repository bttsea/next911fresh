...
var _amphtmlValidator = _interopRequireDefault(require("amphtml-validator"));
...
var _errorDebug = _interopRequireDefault(require("./error-debug"));
...
class DevServer extends _nextServer.default {
  constructor(options) {
    ...
    this.renderOpts.ErrorDebug = _errorDebug.default;
    this.devReady = new Promise(resolve => {
      this.setDevReady = resolve;
    });
    this.renderOpts.ampValidator = (html, pathname) => {
      return _amphtmlValidator.default.getInstance().then(validator => {
    ...
    if (_fs.default.existsSync((0, _path.join)(this.dir, 'static'))) {
      console.warn(`The static directory has been deprecated in favor of the public directory. https://err.sh/zeit/next.js/static-dir-deprecated`);
    }
    ...
    return new Promise(resolve => {
      ...
      wp.on('aggregated', () => {
        ...
        for (const [fileName, {
          ...
          pageName = pageName.replace(new RegExp(`\\.+(?:${this.nextConfig.pageExtensions.join('|')})$`), '');
          pageName = pageName.replace(/\/index$/, '') || '/';
          if (!(0, _utils.isDynamicRoute)(pageName)) {
            continue;
          }
          dynamicRoutedPages.push(pageName);
        }
        this.dynamicRoutes = (0, _utils.getSortedRoutes)(dynamicRoutedPages).map(page => ({
          page,
  ...
  async prepare() {
    await (0, _verifyTypeScriptSetup.verifyTypeScriptSetup)(this.dir, this.pagesDir);
    ...
    this.setDevReady();
    (0, _storage.setDistDir)(this.distDir);
  ...
  async run(req, res, parsedUrl) {
    ...
    if (pathname.startsWith('/_next')) {
      try {
        await fsStat((0, _path.join)(this.publicDir, '_next'));
    ...
    if (await this.hasPublicFile(pathname)) {
      const pageFile = await (0, _findPageFile.findPageFile)(this.pagesDir, (0, _normalizePagePath.normalizePagePath)(pathname), this.nextConfig.pageExtensions);
      if (pageFile) {
        const err = new Error(`A conflicting public file and page file was found for path ${pathname} https://err.sh/zeit/next.js/conflicting-public-file-page`);
        res.statusCode = 500;
  ...
  async renderErrorToHTML(err, req, res, pathname, query) {
    ...
    if (!err && res.statusCode === 500) {
      err = new Error('An undefined error was thrown sometime during render... ' + 'See https://err.sh/zeit/next.js/threw-undefined');
    }
    try {
  ...
  servePublic(req, res, path) {
    const p = (0, _path.join)(this.publicDir, path);
    return this.serveStatic(req, res, p);
  }
  async hasPublicFile(path) {
    try {
      const info = await fsStat((0, _path.join)(this.publicDir, path));
      return info.isFile();
    } catch (_) {
      return false;
    }
