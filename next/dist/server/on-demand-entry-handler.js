...
function addEntry(compilation, context, name, entry) {
  return new Promise((resolve, reject) => {
    const dep = _DynamicEntryPlugin.default.createDependency(entry, name);
...
function onDemandEntryHandler(devMiddleware, multiCompiler, {
  ...
  let reloading = false;
  let stopped = false;
  let reloadCallbacks = new _events.EventEmitter();
  let lastEntry = null;
  for (const compiler of compilers) {
    compiler.hooks.make.tapPromise('NextJsOnDemandEntries', compilation => {
      ...
      const allEntries = Object.keys(entries).map(async page => {
        if (compiler.name === 'client' && page.match(_constants.API_ROUTE)) {
          return;
        }
        const {
          name,
        ...
        return addEntry(compilation, compiler.context, name, [compiler.name === 'client' ? `next-client-pages-loader?${(0, _querystring.stringify)({
          page,
          absolutePagePath
        })}!` : absolutePagePath]);
      });
  ...
  function findHardFailedPages(errors) {
    return errors.filter(e => {
      ...
      if (_constants2.IS_BUNDLED_PAGE_REGEX.test(e.module.name)) return true;

      // No dependencies means this is a top level page.
      // So this is a failed page.
      return e.module.dependencies.length === 0;
    }).map(e => e.module.chunks).reduce((a, b) => [...a, ...b], []).map(c => {
      const pageName = _constants2.ROUTE_NAME_REGEX.exec(c.name)[1];
  ...
  function getPagePathsFromEntrypoints(entrypoints) {
    ...
    for (const [, entrypoint] of entrypoints.entries()) {
      const result = _constants2.ROUTE_NAME_REGEX.exec(entrypoint.name);
      if (!result) {
        continue;
      }
