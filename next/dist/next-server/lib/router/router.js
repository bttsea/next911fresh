...
} = require('./rewrite-url-for-export');
const {
  getRouteMatcher
} = require('./utils/route-matcher');
const {
  getRouteRegex
} = require('./utils/route-regex');
const {
  isDynamicRoute
} = require('./utils/is-dynamic');
...
class Router {
  constructor(pathname, query, as, options) {
    ...
    this.query = query;
    this.asPath = isDynamicRoute(pathname) && __NEXT_DATA__.nextExport ? pathname : as;
  ...
  getRouteInfo(route, pathname, query, as, shallow = false) {
    ...
    return Promise.resolve().then(() => {
      ...
      return this.fetchComponent(route).then(Component => ({
        Component
      }));
    }).then(routeInfo => {
      const {
