...
const getModernOptions = (babelOptions = {}) => {
  ...
  const transformRuntimeOptions = Object.assign({}, babelOptions['transform-runtime'], {
    regenerator: false
  });
  presetEnvOptions.targets = {
    esmodules: true
  };
  presetEnvOptions.exclude = [...(presetEnvOptions.exclude || []),
...
const nextBabelPresetModern = presetOptions => context => nextBabelPreset(context, getModernOptions(presetOptions));
module.exports = _babelLoader.default.custom(babel => {
  ...
  const commonJsItem = babel.createConfigItem(require('@babel/plugin-transform-modules-commonjs'), {
    type: 'plugin'
  });
  ...
  return {
    customOptions(opts) {
      ...
      const loader = Object.assign(opts.cache ? {
        ...
        cacheIdentifier: cacheKey + (opts.isServer ? '-server' : '') + (opts.isModern ? '-modern' : '') + (opts.hasModern ? '-has-modern' : '') + JSON.stringify(babel.loadPartialConfig({
          filename,
          cwd: opts.cwd,
    ...
    config(cfg, {
      ...
      if (!isServer && isPageFile) {
        const pageConfigPlugin = babel.createConfigItem([require('../../babel/plugins/next-page-config')], {
          type: 'plugin'
        });
        options.plugins.push(pageConfigPlugin);
      }
      if (isServer && source.indexOf('next/data') !== -1) {
        const nextDataPlugin = babel.createConfigItem([require('../../babel/plugins/next-data'), {
          key: (0, _path.basename)(filename) + '-' + (0, _stringHash.default)(filename)
        }], {
          type: 'plugin'
        });
      ...
      if (isModern) {
        const nextPreset = options.presets.find(preset => preset && preset.value === nextBabelPreset) || {
          options: {}
        };
        const additionalPresets = options.presets.filter(preset => preset !== nextPreset);
        const presetItemModern = babel.createConfigItem(nextBabelPresetModern(nextPreset.options), {
          type: 'preset'
        });
      ...
      options.plugins.push([require.resolve('babel-plugin-transform-define'), {
        'typeof window': isServer ? 'undefined' : 'object'
      }, 'next-js-transform-define-instance']);

      // As next-server/lib has stateful modules we have to transpile commonjs
      options.overrides = [...(options.overrides || []), {
        test: [/next[\\/]dist[\\/]next-server[\\/]lib/, /next[\\/]dist[\\/]client/, /next[\\/]dist[\\/]pages/],
