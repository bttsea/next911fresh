...
var _nextPageConfig = require("../../../../babel/plugins/next-page-config");
...
class TerserPlugin {
  ...
  apply(compiler) {
    const optimizeFn = (compilation, chunks, callback) => {
      ...
      const tasks = [];
      Array.from(chunks).reduce((acc, chunk) => acc.concat(chunk.files || []), []).concat(compilation.additionalChunkAssets || []).filter(file => JS_REGEX.test(file)).forEach(file => {
        let inputSourceMap;
        ...
        try {
          ...
          if (compress) {
            ...
            compress.global_defs['self.__HAS_SPR'] = !!_nextPageConfig.sprStatus.used;
          }
          const task = {
            file,
            input,
          ...
          if (this.options.cache) {
            ...
            task.cacheKey = 'a' + (0, _stringHash.default)(input);
            if (this.options.sourceMap) task.cacheKey += 's';
          }
          tasks.push(task);
        } catch (error) {
          compilation.errors.push(TerserPlugin.buildError(error, file, TerserPlugin.buildSourceMap(inputSourceMap), new _webpack.RequestShortener(compiler.context)));
        }
      });
      taskRunner.run(tasks, (tasksError, results) => {
        ...
        results.forEach((data, index) => {
          ...
          if (error) {
            compilation.errors.push(TerserPlugin.buildError(error, file, sourceMap, new _webpack.RequestShortener(compiler.context)));
            return;
          }
          let outputSource;
          if (map) {
          ...
          if (warnings && warnings.length > 0) {
            warnings.forEach(warning => {
              const builtWarning = TerserPlugin.buildWarning(warning, file, sourceMap, new _webpack.RequestShortener(compiler.context), this.options.warningsFilter);
              if (builtWarning) {
