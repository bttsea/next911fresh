...
exports.sprStatus = exports.dropBundleIdentifier = void 0;
const dropBundleIdentifier = exports.dropBundleIdentifier = '__NEXT_DROP_CLIENT_FILE__';
const sprStatus = exports.sprStatus = {
  used: false
};
...
function replaceBundle(path, t) {
  path.parentPath.replaceWith(t.program([t.variableDeclaration('const', [t.variableDeclarator(t.identifier('config'), t.assignmentExpression('=', t.identifier(dropBundleIdentifier), t.stringLiteral(`${dropBundleIdentifier} ${Date.now()}`)))])], []));
}
    ...
    visitor: {
        ...
        enter(path, state) {
          path.traverse({
            ExportNamedDeclaration(path, state) {
              ...
              if (id && (id.name === EXPORT_NAME_GET_STATIC_PROPS || id.name === EXPORT_NAME_GET_STATIC_PARAMS)) {
              ...
              for (const declaration of declarations) {
                ...
                if (declaration.init.type !== 'ObjectExpression') {
                  ...
                  throw new Error(`Invalid page config export found. Expected object but got ${declaration.init.type} in file ${pageName}. See: https://err.sh/zeit/next.js/invalid-page-config`);
                }
      ...
      ExportDefaultDeclaration(path, state) {
        ...
        if (prev.type.endsWith('Declaration')) {
          prev.type = prev.type.replace(/Declaration$/, 'Expression');
        }
        path.insertBefore([t.variableDeclaration('const', [t.variableDeclarator(t.identifier(pageComponentVar), prev)]), t.assignmentExpression('=', t.memberExpression(t.identifier(pageComponentVar), t.identifier(prerenderId)), t.booleanLiteral(true))]);
