        ...
        exit(path, state) {
          if (state.get('jsx')) {
            ...
            if (state.opts.property) {
              ...
              const mapping = t.variableDeclaration('var', [t.variableDeclarator(pragma, t.memberExpression(importAs, t.identifier(state.opts.property)))]);
            ...
            if (!existingBinding) {
              const importSpecifier = t.importDeclaration([state.opts.import ?
              // import { $import as _pragma } from '$module'
              t.importSpecifier(importAs, t.identifier(state.opts.import)) : state.opts.importNamespace ? t.importNamespaceSpecifier(importAs) :
              // import _pragma from '$module'
              t.importDefaultSpecifier(importAs)], t.stringLiteral(state.opts.module || 'react'));
