...
var _codeframeFormatter = require("fork-ts-checker-webpack-plugin/lib/formatter/codeframeFormatter");
...
var WebpackStatusPhase = /*#__PURE__*/function (WebpackStatusPhase) {
  ...
  WebpackStatusPhase[WebpackStatusPhase["TYPE_CHECKING"] = 3] = "TYPE_CHECKING";
...
function ampValidation(page, errors, warnings) {
  ...
  if (!(errors.length || warnings.length)) {
    buildStore.setState({
      amp: Object.keys(amp).filter(k => k !== page).sort().reduce((a, c) => (a[c] = amp[c], a), {})
    });
    return;
  }
  const newAmp = {
  ...
  buildStore.setState({
    amp: Object.keys(newAmp).sort().reduce((a, c) => (a[c] = newAmp[c], a), {})
  });
}
function watchCompilers(client, server, enableTypeCheckingOnClient, onTypeChecked) {
  ...
  function tapCompiler(key, compiler, hasTypeChecking, onEvent) {
    ...
    if (hasTypeChecking) {
      const typescriptFormatter = (0, _codeframeFormatter.createCodeframeFormatter)({});
      compiler.hooks.beforeCompile.tap(`NextJs-${key}-StartTypeCheck`, () => {
        tsMessagesPromise = new Promise(resolve => {
          tsMessagesResolver = msgs => resolve(msgs);
        });
      });
      _forkTsCheckerWebpackPlugin.default.getCompilerHooks(compiler).receive.tap(`NextJs-${key}-afterTypeScriptCheck`, (diagnostics, lints) => {
        const allMsgs = [...diagnostics, ...lints];
        const format = message => typescriptFormatter(message, true);
        const errors = allMsgs.filter(msg => msg.severity === 'error').map(d => ({
          file: (d.file || '').replace(/\\/g, '/'),
          message: format(d)
        }));
        const warnings = allMsgs.filter(msg => msg.severity === 'warning').map(d => ({
          file: (d.file || '').replace(/\\/g, '/'),
          message: format(d)
        }));
        tsMessagesResolver({
    ...
    compiler.hooks.done.tap(`NextJsDone-${key}`, stats => {
      ...
      if (!hasErrors && typePromise) {
        typePromise.then(typeMessages => {
          ...
          const reportFiles = stats.compilation.modules.map(m => (m.resource || '').replace(/\\/g, '/')).filter(Boolean);
          let filteredErrors = typeMessages.errors ? typeMessages.errors.filter(({
            file
          }) => file && reportFiles.includes(file)).map(({
            message
          }) => message) : null;
          if (filteredErrors && filteredErrors.length < 1) {
            filteredErrors = null;
          }
          let filteredWarnings = typeMessages.warnings ? typeMessages.warnings.filter(({
            file
          }) => file && reportFiles.includes(file)).map(({
            message
          }) => message) : null;
  ...
  tapCompiler('client', client, enableTypeCheckingOnClient, status => buildStore.setState({
    client: status
  }));
  tapCompiler('server', server, false, status => buildStore.setState({
    server: status
  }));
