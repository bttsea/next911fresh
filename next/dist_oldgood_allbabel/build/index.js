...
var _chunkGraphPlugin = require("./webpack/plugins/chunk-graph-plugin");
...
async function build(dir, conf = null) {
  ...
  (0, _storage.setDistDir)(distDir);
  const publicDir = _path.default.join(dir, 'public');
  const pagesDir = (0, _findPagesDir.findPagesDir)(dir);
  let publicFiles = [];
  let hasPublicDir = false;
  let backgroundWork = [];
  backgroundWork.push((0, _events.recordVersion)({
    cliCommand: 'build'
  }), (0, _events.recordNextPlugins)(_path.default.resolve(dir)));
  ...
  const pagePaths = await (0, _utils2.collectPages)(pagesDir, config.pageExtensions);
  ...
  const entrypoints = (0, _entries.createEntrypoints)(mappedPages, target, buildId, config);
  ...
  for (let file of publicFiles) {
    file = file.replace(/\\/g, '/').replace(/\/index$/, '').split(publicDir).pop();
    if (mappedPages[file]) {
  ...
  if (numConflicting) {
    throw new Error(`Conflicting public and page file${numConflicting === 1 ? ' was' : 's were'} found. https://err.sh/zeit/next.js/conflicting-public-file-page\n${conflictingPublicFiles.join('\n')}`);
  }
  const configs = await Promise.all([(0, _webpackConfig.default)(dir, {
    tracer,
    buildId,
    isServer: false,
    config,
  ...
  if (clientConfig.optimization && (clientConfig.optimization.minimize !== true || clientConfig.optimization.minimizer && clientConfig.optimization.minimizer.length === 0)) {
    console.warn(_chalk.default.bold.yellow(`Warning: `) + _chalk.default.bold(`Production code optimization has been disabled in your project. Read more: https://err.sh/zeit/next.js/minification-disabled`));
  }
  ...
  if (result.errors.length > 0) {
    ...
    backgroundWork.push((0, _events.recordBuildDuration)({
  ...
  const manifestPath = _path.default.join(distDir, isLikeServerless ? _constants2.SERVERLESS_DIRECTORY : _constants2.SERVER_DIRECTORY, _constants2.PAGES_MANIFEST);
  ...
  await Promise.all(pageKeys.map(async page => {
    const chunks = (0, _chunkGraphPlugin.getPageChunks)(page);
    const actualPage = page === '/' ? '/index' : page;
    const size = await (0, _utils2.getPageSizeInKb)(actualPage, distDir, buildId, buildManifest, config.experimental.modern);
    const bundleRelative = _path.default.join(isLikeServerless ? 'pages' : `static/${buildId}/pages`, actualPage + '.js');
    const serverBundle = _path.default.join(distDir, isLikeServerless ? _constants2.SERVERLESS_DIRECTORY : _constants2.SERVER_DIRECTORY, bundleRelative);
    let isStatic = false;
    ...
    if (nonReservedPage && customAppGetInitialProps === undefined) {
      customAppGetInitialProps = (0, _utils2.hasCustomAppGetInitialProps)(isLikeServerless ? serverBundle : _path.default.join(distDir, _constants2.SERVER_DIRECTORY, `/static/${buildId}/pages/_app.js`), runtimeEnvConfig);
      if (customAppGetInitialProps) {
        console.warn(_chalk.default.bold.yellow(`Warning: `) + _chalk.default.yellow(`You have opted-out of Automatic Static Optimization due to \`getInitialProps\` in \`pages/_app\`.`));
        console.warn('Read more: https://err.sh/next.js/opt-out-auto-static-optimization\n');
      }
    }
    if (nonReservedPage) {
      try {
  ...
  if (invalidPages.size > 0) {
    throw new Error(`automatic static optimization failed: found page${invalidPages.size === 1 ? '' : 's'} without a React Component as default export in \n${[...invalidPages].map(pg => `pages${pg}`).join('\n')}\n\nSee https://err.sh/zeit/next.js/page-without-valid-component for more info.\n`);
  }
  ...
  if (staticPages.size > 0 || sprPages.size > 0) {
    ...
    const moveExportedPage = async (page, file, isSpr, ext) => {
      ...
      const relativeDest = (isLikeServerless ? _path.default.join('pages', file) : _path.default.join('static', buildId, 'pages', file)).replace(/\\/g, '/');
      const dest = _path.default.join(distDir, isLikeServerless ? _constants2.SERVERLESS_DIRECTORY : _constants2.SERVER_DIRECTORY, relativeDest);
      if (!isSpr) {
      ...
      if (isSpr) {
        ...
        if (!isDynamic) {
          ...
          finalPrerenderRoutes[page] = {
            ...
            srcRoute: null,
            dataRoute: _path.default.posix.join('/_next/data', `${page === '/' ? '/index' : page}.json`)
          };
        } else {
          ...
          for (const route of extraRoutes) {
            ...
            finalPrerenderRoutes[route] = {
              ...
              srcRoute: page,
              dataRoute: _path.default.posix.join('/_next/data', `${route === '/' ? '/index' : route}.json`)
            };
          }
        }
      }
  ...
  backgroundWork.push((0, _events.recordBuildOptimize)({
  ...
  if (sprPages.size > 0) {
    ...
    tbdPrerenderRoutes.forEach(tbdRoute => {
      const dataRoute = _path.default.posix.join('/_next/data', `${tbdRoute === '/' ? '/index' : tbdRoute}.json`);
      finalDynamicRoutes[tbdRoute] = {
        routeRegex: (0, _utils.getRouteRegex)(tbdRoute).re.source,
        dataRoute,
        dataRouteRegex: (0, _utils.getRouteRegex)(dataRoute.replace(/\.json$/, '')).re.source.replace(/\(\?:\\\/\)\?\$$/, '\\.json$')
      };
    });
    ...
    await fsWriteFile(_path.default.join(distDir, _constants2.PRERENDER_MANIFEST), JSON.stringify(prerenderManifest), 'utf8');
  }
  staticPages.forEach(pg => allStaticPages.add(pg));
  pageInfos.forEach((info, key) => {
    allPageInfos.set(key, info);
  });
  (0, _utils2.printTreeView)(Object.keys(mappedPages), allPageInfos, isLikeServerless);
  if (tracer) {
