...
exports.hasCustomAppGetInitialProps = hasCustomAppGetInitialProps;
...
const fsStatPromise = (0, _util.promisify)(_fs.default.stat);
const fileStats = {};
const fsStat = file => {
...
function printTreeView(list, pageInfos, serverless) {
  const getPrettySize = _size => {
    const size = (0, _prettyBytes.default)(_size);
    // green for 0-100kb
    if (_size < 100 * 1000) return _chalk.default.green(size);
    // yellow for 100-250kb
    if (_size < 250 * 1000) return _chalk.default.yellow(size);
    // red for >= 250kb
    return _chalk.default.red.bold(size);
  };
  const messages = [['Page', 'Size', 'Files', 'Packages'].map(entry => _chalk.default.underline(entry))];
  list.sort((a, b) => a.localeCompare(b)).forEach((item, i) => {
    const symbol = i === 0 ? list.length === 1 ? '─' : '┌' : i === list.length - 1 ? '└' : '├';
    const pageInfo = pageInfos.get(item);
    messages.push([`${symbol} ${item.startsWith('/_') ? ' ' : pageInfo && pageInfo.static ? _chalk.default.bold('⚡') : serverless ? 'λ' : 'σ'} ${item}`, ...(pageInfo ? [pageInfo.isAmp ? _chalk.default.cyan('AMP') : pageInfo.size >= 0 ? getPrettySize(pageInfo.size) : '', pageInfo.chunks ? pageInfo.chunks.internal.size.toString() : '', pageInfo.chunks ? pageInfo.chunks.external.size.toString() : ''] : ['', '', ''])]);
  });
  ...
  console.log();
  console.log((0, _textTable.default)([serverless ? ['λ', '(Lambda)', `page was emitted as a lambda (i.e. ${_chalk.default.cyan('getInitialProps')})`] : ['σ', '(Server)', `page will be server rendered (i.e. ${_chalk.default.cyan('getInitialProps')})`], [_chalk.default.bold('⚡'), '(Static File)', 'page was prerendered as static HTML']], {
    align: ['l', 'l', 'l'],
    stringLength: str => (0, _stripAnsi.default)(str).length
  }));
  console.log();
...
async function getPageSizeInKb(page, distPath, buildId, buildManifest, isModern) {
  const clientBundle = _path.default.join(distPath, `static/${buildId}/pages/`, `${page}${isModern ? '.module' : ''}.js`);
  ...
  const deps = (buildManifest.pages[page] || []).filter(dep => !baseDeps.includes(dep) && /\.module\.js$/.test(dep) === isModern).map(dep => `${distPath}/${dep}`);
...
async function isPageStatic(page, serverBundle, runtimeEnvConfig) {
  try {
    ...
    if (hasGetInitialProps && hasStaticProps) {
      throw new Error(_constants.SPR_GET_INITIAL_PROPS_CONFLICT);
    }

    // A page cannot have static parameters if it is not a dynamic page.
    if (hasStaticProps && hasStaticParams && !(0, _isDynamic.isDynamicRoute)(page)) {
      throw new Error(`unstable_getStaticParams can only be used with dynamic pages. https://nextjs.org/docs#dynamic-routing`);
    }
    let prerenderPaths;
        ...
        // required keys.
          ...
          _validParamKeys.forEach(validParamKey => {
            if (typeof entry[validParamKey] !== 'string') {
              throw new Error(`A required parameter (${validParamKey}) was not provided as a string.`);
            }
            builtPage = builtPage.replace(`[${validParamKey}]`, encodeURIComponent(entry[validParamKey]));
          });
...
function hasCustomAppGetInitialProps(_appBundle, runtimeEnvConfig) {
  require('../next-server/lib/runtime-config').setConfig(runtimeEnvConfig);
