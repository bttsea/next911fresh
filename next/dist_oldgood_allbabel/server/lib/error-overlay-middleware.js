...
function errorOverlayMiddleware(options) {
  return (req, res, next) => {
    if (req.url.startsWith('/_next/development/open-stack-frame-in-editor')) {
      ...
      if (!_fs.default.existsSync(resolvedFileName)) {
        resolvedFileName = _path.default.join(options.dir, resolvedFileName);
      }
      (0, _launchEditor.default)(`${resolvedFileName}:${lineNumber}:${colNumber}`);
      res.end();
    } else {
      next();
    }
