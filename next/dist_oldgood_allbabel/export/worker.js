...
const writeFileP = (0, _util.promisify)(_fs.writeFile);
const mkdirp = (0, _util.promisify)(_mkdirp.default);
const accessP = (0, _util.promisify)(_fs.access);
...
async function _default({
  ...
  try {
    ...
    if ((0, _isDynamic.isDynamicRoute)(page) && page !== path) {
      const params = (0, _routeMatcher.getRouteMatcher)((0, _routeRegex.getRouteRegex)(page))(path);
      if (params) {
        query = {
          ...query,
          ...params
    ...
    const htmlFilepath = (0, _path.join)(outDir, htmlFilename);
    ...
    const renderedDuringBuild = unstable_getStaticProps => {
      // eslint-disable-next-line camelcase
      return !buildExport && unstable_getStaticProps && !(0, _isDynamic.isDynamicRoute)(path);
    };
    if (serverless) {
      const mod = require((0, _path.join)(distDir, 'serverless/pages', (page === '/' ? 'index' : page) + '.js'));
      ...
      const components = await (0, _loadComponents.loadComponents)(distDir, buildId, page, serverless);
    ...
    if (curRenderOpts.inAmpMode) {
      ...
      try {
        ...
        if (serverless) {
          ...
          ampHtml = await renderMethod(req, res, page, {
            ...query,
            amp: 1
          }, curRenderOpts);
        }
    ...
    if (curRenderOpts.sprData) {
      const dataFile = (0, _path.join)(sprDataDir, htmlFilename.replace(/\.html$/, '.json'));
      await mkdirp((0, _path.dirname)(dataFile));
